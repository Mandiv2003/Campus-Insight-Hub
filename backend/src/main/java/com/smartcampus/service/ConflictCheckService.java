...
public class ConflictCheckService {
    ...
    /**
     * Throws {@link ConflictException} if an APPROVED booking already overlaps
     * the proposed time window for the given resource.
     *
     * <p>Overlap condition: {@code existing.start < proposed.end AND existing.end > proposed.start}
     *
     * @param excludeBookingId pass {@code null} for new bookings; pass the booking's own ID when
     *                         re-checking at approval time so it does not conflict with itself
     */
